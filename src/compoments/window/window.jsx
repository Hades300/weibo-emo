...
const label2Badge = (label) => {
    let text = label2Text(label)
    if (text==""){
        return null
    }
    let color = label2Color(label)
    return <span className="window-emotion-badge" style={{backgroundColor:color,color:"#fff",padding:".25rem .5rem",borderRadius:".75rem"}}>{text}</span>
}
...
const Window  = ({keyword,count,state={}},ref) =>{
  ...
  return (
    <div className="window-container" ref={ref}>
        ...
        {
            Array.prototype.map.call(cards,({text,id,like_count,repost_count,utc_created,uid,desktop_url,reason,sentiment_distribution},index)=>{
                return (
                    <div className="window-item" key={id}>
                        <div className="window-item-content" dangerouslySetInnerHTML={{__html:text}}></div>
                        <div className="window-item-bottom">
                            <div className="window-item-badge">
                                <span className='window-item-weibo' onClick={()=>{openWindow(desktop_url)}}>{weiboIcon}</span>
                        {sentiment_distribution && sentiment_distribution.length!=0  && <span className="window-item-emotion">{label2Badge(sentiment_distribution[0]?.name)}</span>}
